...
export default function DateRangePicker({ value, onChange }: DateRangePickerProps) {
  ...
  const presets = [
    { label: t('Today'),         getRange: () => { const d = new Date(); return { from: d, to: d }; } },
    { label: t('Last 7 days'),   getRange: () => { const to = new Date(); const from = new Date(); from.setDate(from.getDate() - 6); return { from, to }; } },
    { label: t('Last 30 days'),  getRange: () => { const to = new Date(); const from = new Date(); from.setDate(from.getDate() - 29); return { from, to }; } },
    { label: t('Month to date'), getRange: () => { const to = new Date(); const from = new Date(to.getFullYear(), to.getMonth(), 1); return { from, to }; } },
    { label: t('Year to date'),  getRange: () => { const to = new Date(); const from = new Date(to.getFullYear(), 0, 1); return { from, to }; } },
    { label: t('All time'),      getRange: () => ({ from: null, to: null }) },
  ];
  ...
  const triggerLabel = () => {
    if (!value.from && !value.to) return t('All time');
    if (value.from && !value.to)  return formatDate(value.from, locale);
    if (value.from && value.to) {
      if (isSameDay(value.from, value.to)) return formatDate(value.from, locale);
      return `${formatDate(value.from, locale)} – ${formatDate(value.to, locale)}`;
    }
    return t('Select range');
  };

  const pendingLabel = () => {
    if (!pendingRange.from && !pendingRange.to) return t('All time');
    if (pendingRange.from && !pendingRange.to)   return `${formatDate(pendingRange.from, locale)} → ${t('pick end date')}`;
    if (pendingRange.from && pendingRange.to)     return `${formatDate(pendingRange.from, locale)} – ${formatDate(pendingRange.to, locale)}`;
    return '';
  };
